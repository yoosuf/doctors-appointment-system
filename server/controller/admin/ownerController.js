...
const { assignPermissionOwner } = require("../../services/seeder");
...
const addUser = async (req, res) => {
  try {
    ...
    if (findUser && findUser.email == body.email) {
      ...
      const adminRoleId = await Role.distinct("_id", {
        code: {
          $in: [ROLE.ADMIN, ROLE.SUPER_ADMIN],
        },
      });
      const parentIds = await user.distinct("_id", {
        roleId: adminRoleId,
      });
      body.parentId = parentIds;
      ...
      assignPermissionOwner(result._id);
      res.message =
        "Owner " +
        result.firstName +
        " " +
...
const findAllUser = async (req, res) => {
  try {
    ...
    query.roleId = OwnerRole.id;
    const result = await service.getAllDocuments(user, query, options);
    res.message = "All Owners" + MESSAGE.USER_LIST.message;
