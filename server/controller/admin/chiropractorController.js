...
const { assignPermissionChiropractor } = require("../../services/seeder");
...
const addUser = async (req, res) => {
  try {
    ...
    if (findUser && findUser.email == body.email) {
      ...
      const ChiroRole = await Role.findOne({
        code: ROLE.CHIROPRACTOR,
      });
      req.body.roleId = ChiroRole.id;
      ...
      const location = await Location.findOne({ _id: locationIds }).lean();
      const data = {
        $addToSet: {
          assignee: result._id,
        },
      ...
      assignPermissionChiropractor(result._id);
      res.message =
        "Chiropractor " +
        result.firstName +
        " " +
...
const findAllUser = async (req, res) => {
  try {
    ...
    query.roleId = ChiroRole.id;
    const result = await service.getAllDocuments(user, query, options);
    res.message = "All ChiroPractors" + MESSAGE.USER_LIST.message;
    return utils.successResponse(result, res);
  } catch (error) {
...
const updateUser = async (req, res) => {
  try {
    ...
    if (userData == null) {
      let message = _localize("auth.user", req);
...
const softDeleteUser = async (req, res) => {
  ...
  res.message = "Chiropractor" + MESSAGE.USER_DELETED.message;
  utils.successResponse(result, res);
};
