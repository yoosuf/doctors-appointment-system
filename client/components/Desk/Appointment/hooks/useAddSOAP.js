  ...
  const getEditData = async id => {
    ...
    try {
      await commonApi({
        ...
        setPatientVisible(data.patientVisible)
        setEditSOAP()
        const viewableData = viewableOptions.find(
          (option = {}) => option.value === data.viewableBy
        )
        setSelectedViewable(viewableData)
      })
    } finally {
      setLoading(false)
    }
