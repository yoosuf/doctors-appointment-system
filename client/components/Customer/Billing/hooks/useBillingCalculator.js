...
const useBillingCalculator = ({ userObj, serviceIds, services }) => {
  ...
  const getCustomerPackageInfo = async () => {
    const isMembershipEmpty = JSON.stringify(userObj?.membership) === '{}'

    if (isMembershipEmpty) {
      try {
        console.log('Fetching patient data')
        const patientData = await commonApi({ action: 'getPatientObject' })
        console.log('Patient data:', patientData)
        const formattedMembership = formattedMembershipObj(
        ...
        console.log('fetchAndFormatMembershipData complete')
      }
      return // Early return
    }
    ...
    const formattedMembership = formattedMembershipObj(userObj?.membership)
    setActiveSubscription(formattedMembership)
    // setInvitationReward({})
  }
    ...
    serviceIds?.forEach(serviceId => {
      ...
      if (selectedService) {
        ...
        const matchedSubscriptionService = activeSubscription?.categories?.find(
          sub => sub._id === selectedService.categoryId
        )
