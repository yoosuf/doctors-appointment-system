...
import { APPOINTMENT_CHART_TYPE, USER_ROLE_TYPE } from '@/utils/constant'
import Image from 'next/image'
import CalenderIcon from '@/widget/image/CalenderIcon'

const ForceInitialAnimation = ({
  chart = {},
  openEdit = () => {},
  chartTemplateData = [],
  onPinnedChart = () => {},
  ...
  const clinician = chart?.creatorId 
  ...
  return (
    <div
      key={chart.id}
      className='mt-4 overflow-hidden text-sm border single-chart-detail rounded-xl border-grayMid sm:text-md'>
      <div className='flex flex-col justify-between p-5 cursor-pointer sm:flex-row sm:items-center chart-detail-header gap-y-3 bg-grayMid'>
        <div className='flex flex-wrap items-center gap-x-3 gap-y-2'>
          <div className='flex items-center'>
            <button
              onClick={e => onPinnedChart(e, chart)}
              disabled={getUser()?.id === clinician?.id ? false : true}
              >
              <svg
            ...
            <h4 className='ml-3'>
              ...
              {chart.patientId?.firstName + ' ' + chart.patientId?.lastName}
            </h4>
          </div>
          {chart.patientVisible && (
            <a className='px-4 py-1 text-xs font-medium rounded-full sm:text-sm text-primary bg-yellowBg'>
              visible to patient
            </a>
          )}
        </div>
        ...
        <div
          className={`flex gap-3 sm:gap-5 ${
            clinician?.id === getUser()?.id
              ? 'flex-col sm:flex-row sm:items-center'
              : 'justify-between'
          }`}>
          <div className='flex items-center gap-4'>
            {getUser()?.id === clinician?.id ? (
              <a onClick={() => openEdit(chart)}>
                <EditIcon color='#9ca3af' className='w-5 h-5' />
              </a>
            ) : (
              ''
            )}
            ...
            <Link href='#'>
              <a className='flex items-center chart-profile'>
                <img
                  src={
                    baseUrl + clinician?.profile_image?.uri
                  }
          ...
          <div
            className={`collpse-arrow gap-5 sm:gap-3 ${
              getUser()?.roleId?.code === USER_ROLE_TYPE.CHIROPRACTOR ||
              getUser()?.roleId?.code === USER_ROLE_TYPE.NURSE ||
              getUser()?.roleId?.code === USER_ROLE_TYPE.STAFF
            ...
            <a onClick={() => setIsOpened(!isOpened)}>
...
export default React.memo(ForceInitialAnimation)
