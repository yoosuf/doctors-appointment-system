...
function MimimalProfile({searchResultData, activityList, height,weight, notes , chartList, emergencyContact, visitReason, discomfort, refferalInformation, aggravates, painScale, onBoardingAnswer, loader, chartLoader} ) {
    return (
        <>
                                ...
                                <div>
                                    ...
                                    <p className='mt-1 text-sm text-gray-400'>
                                        {searchResultData?.dob
                                            ? moment().diff(searchResultData?.dob, 'years') + ' yr'
                                            : '-'}
                                        /{' '}
                                        {searchResultData?.genderId?.name
                                            ? searchResultData?.genderId?.name
                                            : '-'}
                                    </p>
                                </div>
                                    ...
                                    <div className='flex items-center mt-1'>
                                        {searchResultData?.addressIds?.[0]?.address ? (
                                            <p className='text-sm text-gray-400'>
                                                {searchResultData?.addressIds?.[0]?.address?.addressLine1}{' '}
                                                {searchResultData?.addressIds?.[0]?.address?.addressLine2}{' '}
                                                {searchResultData?.addressIds?.[0]?.address?.cityNm}{' '}
                                                {searchResultData?.addressIds?.[0]?.address?.provinceNm}{' '}
                                                {searchResultData?.addressIds?.[0]?.address?.countryId?.name}{' '}
                                                {searchResultData?.addressIds?.[0]?.address?.postalCodeNm}
                                            </p>
                                        ) : (
